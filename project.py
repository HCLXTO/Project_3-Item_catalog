...
@app.route('/mido')
def showMido():
	return render_template('mido.html')
...
def showItems():
	...
	return render_template('main.html', STATE = state, user = user, items = items, category = False)

@app.route('/category', methods=['GET'])
def showCategory():
	if request.args.get('state') == login_session['state']:#check if the state is correct
		user = dbInterface.getUserByEmail(login_session['email']) if login_session.get('email') else False
		state = login_session['state']
		category = request.args.get('category')
		items = dbInterface.getItemsByCategory(category)

		return render_template('main.html', STATE = state, user = user, items = items, category = category)
	else:
		flash('Wrong state.')
...
def newItem():
	if request.args.get('state') == login_session['state']:#check if the state is correct
		if login_session.get('email'):#check if a user is logged
			...
			print 'DATA INFO:'
			print form
			dbInterface.newItem(title=form['name'], category=form['category'], description=form['description'], user_id=user['id'])
...
def editItem():
	if request.args.get('state') == login_session['state']:#check if the state is correct
		if login_session.get('email'):#check if a user is logged
			user = dbInterface.getUserByEmail(login_session['email'])#get the user
			form = json.loads(request.data)#get the form with the item info 
			dbInterface.editItem(item_id=form['id'],title=form['name'], category=form['category'], description=form['description'], user_id=user['id'])
			flash('Item edited.')
			response = make_response(json.dumps("Item edited."), 200)
			return response
		else:
			flash('A user must be logged to edit a item.')
			response = make_response(json.dumps("A user must be logged to edit a item."), 401)
			return response
	else:
		flash('Wrong state.')
		response = make_response(json.dumps("Wrong state."), 401)
...
def deleteItem():
	if request.args.get('state') == login_session['state']:#check if the state is correct
		if login_session.get('email'):#check if a user is logged
			...
			response = make_response(json.dumps("A user must be logged to delete a item."), 401)
			return response
	else:
		flash('Wrong state.')
		response = make_response(json.dumps("Wrong state."), 401)
